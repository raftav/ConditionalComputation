...
class Model(object):
	...
	def __init__(self,features,labels,seq_length,config,is_training):
		...
		with tf.variable_scope('forward_cells'):
			...
			for i in range(config.num_layers):
				with tf.variable_scope('layer_{:d}'.format(i)):
					rnn_cell= hmlstm.HmLstmCell(config.n_hidden)
					rnn_cell.slope=slope

					c_init=tf.get_variable('c_init_state',[1,rnn_cell.state_size.c],
				...
				zero_states_fw.append(hmlstm.HmLstmStateTuple(c_init,h_init,z_init))

				cell_list_fw.append(rnn_cell)

			self._cell_slope = cell_list_fw[0].slope

			multi_cell_fw = hmlstm.MultiHmLstmCell(cell_list_fw)
		...
		with tf.variable_scope('backward_cells'):
			...
			for i in range(config.num_layers):
				...
				zero_states_bw.append(hmlstm.HmLstmStateTuple(c_init,h_init,z_init))

				cell_list_bw.append(rnn_cell)


			multi_cell_bw = hmlstm.MultiHmLstmCell(cell_list_bw)
		...
		with tf.variable_scope("Output_FW"):
			...
			with tf.device('/gpu:0'):
				...
				for i in range(config.num_layers):
					...
					# w^l 
					gating_unit_weight_fw = tf.get_variable("w{}".format(i), shape=[concat_rnn_output_fw.get_shape().as_list()[1],1], 
														dtype=tf.float32,initializer=tf.random_uniform_initializer(minval=-1.0,maxval=1.0))

					# g_t^l 
					# gating unit for layer l 
					gating_unit_fw = tf.sigmoid(tf.matmul(concat_rnn_output_fw,gating_unit_weight_fw))

					#test
					if not is_training:
						gating_weigths_dict_fw['{}'.format(i)] = gating_unit_fw
					...
					output_logit_fw = gating_unit_fw * tf.matmul(rnn_outputs_layer_fw, output_embedding_matrix_fw)
		...
		with tf.variable_scope("Output_BW"):
			...
			with tf.device('/gpu:0'):
				...
				for i in range(config.num_layers):
					...
					# w^l 
					gating_unit_weight_bw = tf.get_variable("w{}".format(i), shape=[concat_rnn_output_bw.get_shape().as_list()[1],1], 
														dtype=tf.float32,initializer=tf.random_uniform_initializer(minval=-1.0,maxval=1.0))

					# g_t^l 
					# gating unit for layer l 
					gating_unit_bw = tf.sigmoid(tf.matmul(concat_rnn_output_bw,gating_unit_weight_bw))

					#test
					if not is_training:
						gating_weigths_dict_bw['{}'.format(i)] = gating_unit_bw
					...
					# output from a single layer
					output_logit_bw = gating_unit_bw * tf.matmul(rnn_outputs_layer_bw, output_embedding_matrix_bw)
		...
		if is_training:
			...
			with tf.name_scope('optimizer'):
				...
				clip_grad  = [None if gradient is None else tf.clip_by_norm(gradient, 1.0) for gradient in gradients] 
	...
	@property
	def gating_weights_fw(self):
		return self._gating_weights_fw
	...
	@property
	def gating_weights_bw(self):
		return self._gating_weights_bw
