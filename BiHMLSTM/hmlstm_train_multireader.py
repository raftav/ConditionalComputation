	...
	with tf.Graph().as_default():
		...
		with tf.device('/cpu:0'):
			with tf.variable_scope('model',reuse=None):
				...
				start=time.time()
				train_model = hmlstm_model.Model(audio_features,audio_labels,seq_length,config,is_training=True)
			...
			with tf.variable_scope('model',reuse=True):
				...
				start=time.time()
				val_model = hmlstm_model.Model(audio_features_val,audio_labels_val,seq_length_val,config,is_training=None)
				print('Graph creation time = ',time.time() - start)
				print('done.')
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
			...
			#sess = tf_debug.LocalCLIDebugWrapperSession(sess,dump_root="/DATA_NEW/rtavarone/tfdb/")
