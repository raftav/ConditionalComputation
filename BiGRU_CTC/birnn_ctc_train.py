...
class Configuration(object):
	...
	updating_step=int(sys.argv[3])
	learning_decay=float(sys.argv[4])
	keep_prob=float(sys.argv[5])
	batch_size=int(sys.argv[6])
	optimizer_choice=sys.argv[7]
...
def input_pipeline(filenames,config):
    ...
    sequence_length, labels_length, audio_features, audio_labels,file_key = read_my_file_format(filename_queue,config.audio_feat_dimension)
	...
	with tf.Graph().as_default():
		...
		with tf.device('/gpu:0'):
			with tf.name_scope('train_batch'):
				audio_features, audio_labels, seq_length ,lab_length , files = input_pipeline(filename_train,config)

			with tf.name_scope('validation_batch'):
				audio_features_val, audio_labels_val, seq_length_val , lab_length_val, files_val = \
