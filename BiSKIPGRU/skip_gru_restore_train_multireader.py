...
def restore_train():
	...
	# list of input filenames + check existence
	filename_val=['/home/rtavaron/KaldiTimit/data/DEV/sequence_full_{:04d}.tfrecords'.format(i) \
	...
	with tf.Graph().as_default():
		...
		with tf.device('/gpu:0'):
			with tf.variable_scope('model',reuse=None):
				...
				start=time.time()
				train_model = bi_skip_gru_model.Model(audio_features,audio_labels,seq_length,config,is_training=True)
			...
			with tf.variable_scope('model',reuse=True):
				...
				start=time.time()
				val_model = bi_skip_gru_model.Model(audio_features_val,audio_labels_val,seq_length_val,config,is_training=None)
				print('Graph creation time = ',time.time() - start)
				print('done.')
			...
			try:
				...
				while not coord.should_stop():
					...
					if (step % 50 == 0 or step==1):
						print("step[{:7d}] cost[{:2.5f}] lr[{:1.8f}] slope[{:1.8f}]".format(step,C,LR,SL))
					...
					if ((step % int(num_examples / config.batch_size) == 0) and (step is not 0)):
						...
						if((epoch_counter%plot_every_epoch)==0):
							...
							accuracy , \
							binary_states_fw , binary_states_bw  = sess.run([val_model.accuracy,
																			val_model.binary_states_fw,
																			val_model.binary_states_bw])
								
								
							# get average activation of binary gates at each layer
							skip_average_fw = np.mean(binary_states_fw['z_{:d}'.format(config.num_layers)])
							skip_average_bw = np.mean(binary_states_bw['z_{:d}'.format(config.num_layers)])
							...
							outstring+='{:.5f}'.format(skip_average_fw)
							outstring+='\n'
							testLogFile.write(outstring)

							outstring='{}-bw\t'.format(epoch_counter)
							#average number of boundaries per layer
							outstring+='{:.5f}'.format(skip_average_bw)
							outstring+='\n'
							testLogFile.write(outstring)
							testLogFile.flush()
